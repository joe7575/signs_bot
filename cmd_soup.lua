...
if not minetest.global_exists("bucket") or not minetest.global_exists("xdecor") then return end
...
signs_bot.register_botcommand("take_soup", {
	...
	params = "<slot>",
	num_param = 1,
	description = S("Take boiling soup into empty bowl from cauldron in front of a robot\n(use specified slot number or 0 for auto selection)"),
	check = function(slot)
		slot = tonumber(slot)
		return slot and slot >= 0 and slot <= 8
	end,
	...
	doc.add_entry("signs_bot", "soup", {
		...
		data = {
			...
			text = table.concat({
				...
				S("Cauldon should be empty and located above permanent fire."),
